...
class GlobalState:
    mode = "reality"
    video_source = "http://172.20.10.2:8080/video"
        ...
        if state.is_running:
            ...
            if res_lane.masks is not None:
                try:
                    ...
                    # 0.55
                    roi_y_start, roi_y_end = int(h * 0.4), int(
                        h * 0.95
                    ...
                    if len(x_indices) > 15:
                        ...
                        for y in sample_y:
                            ...
                            # Tính toán vị trí tâm làn đường và trường hợp khi mất dấu 1 bên vạch lane
                        ...
                        if len(center_line_x) >= 4 and state.base_motion_state != "S":
                            ...
                            target_near_x = center_line_x[
                                -1
                            ]  # 🔧 Dùng điểm gần hơn (index -1 thay vì -3) để phản ứng cua nhanh hơn, tránh nhắm vào điểm đã ra ngoài
                            ...
                            if abs(heading_deg) < 3.0 and abs(error_e) < 0.04:
                                error_e = 0
                            # abs(heading_deg) < 3.0 là góc lệch của xe có nhỏ hơn 3 độ hay ko, abs(error_e) là sai số lệch tâm của xe so với lane có nhỏ hơn 0.04 hay ko. Nếu cả 2 điều kiện đều thỏa, tức là xe đang gần như thẳng hàng với lane và không bị lệch nhiều, thì ta sẽ gán error_e = 0 để xe tự tin đi thẳng mà không bị ảnh hưởng bởi những sai số nhỏ.
                            if abs(error_e) < 0.03:
                                error_e = 0
                            ...
                            alpha = 0.85 if abs(steering_angle_deg) > 8 else 0.55
                            steering_angle_deg = (
                                1 - alpha
                            ...
                            if (
                                ...
                                if (  # Nếu góc lái mới lớn hơn hoặc bằng 3 độ so với góc cũ thì mới thực hiện các lệnh bên trong
                                    # để tránh gửi quá nhiều lệnh khi xe đang đi thẳng hoặc chỉ lệch nhỏ
                                    abs(servo_angle - state.last_angle_sent)
                                    >= 3
...
def set_mode(mode: str, file: str = ""):
    # 1. Cập nhật chế độ mới
    state.mode = mode

    # 2. Xác định nguồn video mới dựa trên chế độ
    if mode == "reality":
        new_source = "http://172.20.10.2:8080/video"
    else:
        new_source = f"uploads/{file}"

    # 3. Ghi log trước khi chuyển đổi nguồn ảnh
    add_log(f"🔄 Chuyển chế độ -> {mode.upper()} | Đang cấu hình nguồn video...")

    # 4. Cập nhật vào state chung để `frame_generator` nhận biết và tự re-init cap
    state.video_source = new_source

    return {"status": "ok", "mode": mode, "source": new_source}
